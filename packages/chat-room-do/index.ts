...
export default class ChatRoomDurableObject {
	...
	constructor(controller: DurableObjectState, private env: Env) {
	...
	async handleSession(webSocket: WebSocket, ip: string) {
		...
		let limiter = new RateLimiterClient(
			...
			(err) => webSocket.close(1011, (err as Error).stack)
		);

		// Create our session and add it to the sessions list.
		// We don't send any messages to the client until it has sent us the initial user info
		// message. Until then, we will queue messages in `session.blockedMessages`.
		let session: typeof this.sessions[0] = { webSocket, blockedMessages: [] };
		this.sessions.push(session);

		// Queue "join" messages for all online users, to populate the client's roster.
		this.sessions.forEach((otherSession) => {
			if (otherSession.name) {
				session.blockedMessages = session.blockedMessages || [];
		...
		// client.
		let storage = await this.storage.list<string>({
			reverse: true,
			limit: 100,
		});
